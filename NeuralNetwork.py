...
class NN:
    def __init__(self, structure):
        ...
        for i in range(1, len(self.structure)):
            self.weights.append([[round(np.random.uniform(0,1), 3) for j in range(self.structure[i-1])]
                                 for _ in range(self.structure[i])])
        #print('Weights:', self.weights)

        #Biases
        self.biases = [[round(np.random.uniform(0,1), 3) for node in range(layer)]
    ...
    def calculate(self, inp, activation_function):
      ...
      for layer in range(len(self.values)-1):
        for node in range(len(self.values[layer+1])):
          self.values[layer+1][node] = activation_function(self.biases[layer][node] + sum([x*y for x,y in zip(self.values[layer],self.weights[layer][node])]))
